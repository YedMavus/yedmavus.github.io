...
**Architecture:** Frozen CLIP ViT model used a backbone with learnable task specific LoRA modules.
There is a task shared LoRA module given by $$W = W_0 + BA$$ 
At task t, the update of weight matrix is given by $$W = W_0 + \sum_{i=1}^t B_i A_i$$. 
Output class tokens forwarded thr task specific classifier - gives class scores.

Before training $f_n$, covariance of each class is precomputed based on $f_{n-1}$, where $f_k$ denotes network trained on $k^{th}$ task.
This cov matrices used to align dist of representation generated by current nw with that of old nw.
...
*The class mean shift is found using weighted average of the drift of each sample of that class.*
The Covariance shift is also minimized using $$L_{cov}$$.
...
[^5]: Wu, F., Cheng, L., Tang, S., Zhu, X., Fang, C., Zhang, D. and Wang, M., 2025. Navigating semantic drift in task-agnostic class-incremental learning. arXiv preprint arXiv:2502.07560.
